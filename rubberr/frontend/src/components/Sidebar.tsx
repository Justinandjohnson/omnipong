...
import { Home, MessageSquare, BarChart2, Settings, Trophy, Map as MapIcon } from 'lucide-react';
  ...
  return (
    <div className="h-screen w-64 bg-[var(--sidebar)] border-r border-[#333] flex flex-col fixed left-0 top-0">
      ...
      <nav className="flex-1 px-4 space-y-2">
        ...
        <NavItem icon={<MapIcon size={20} />} label="Map" href="/map" />
        <NavItem icon={<Trophy size={20} />} label="Tournaments" href="/tournaments" />
        <NavItem icon={<BarChart2 size={20} />} label="Analytics" href="/analytics" />
      </nav>
  ...
  return (
    ...
    >
      ...
      <div className={`w-8 h-4 rounded-full relative transition-colors ${isArcadeMode ? 'bg-[var(--rubber-red)]' : 'bg-[#333]'}`}>
        <div className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-all duration-300 ${isArcadeMode ? 'left-4.5' : 'left-0.5'}`} />
      </div>
    </div>
  );
}
