...
def get_user():
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail="User not found in database. Run Sync.")
...
async def signup_tournament(tournament_title: str, recommended_events: list[str] = None):
    try:
        from omnipong_scraper import OmniPongScraper
        scraper = OmniPongScraper(browser_manager)
        
        # If no events provided, we could potentially get them from get_tournament_intelligence
        # but for now we expect the frontend to pass them based on AI insights.
        ...
        result = await scraper.signup_for_tournament(tournament_title, recommended_events)
        return result
    except Exception as e:
...
def get_tournaments(region: str = "local"):
    ...
    try:
        ...
        result = session.execute(text("SELECT title, location, date_range, status, flyer_url FROM activities WHERE activity_type='tournament' ORDER BY id DESC"))
...
def get_notifications():
    ...
    try:
        ...
        result = session.execute(text("SELECT id, type, content, is_read, created_at FROM notifications WHERE is_read = 0 ORDER BY created_at DESC"))
        notifications = []
        for row in result:
             d = dict(row._mapping)
             import json
...
def mark_notification_read(notif_id: int):
    session = SessionLocal()
    try:
        session.execute(text(f"UPDATE notifications SET is_read = 1 WHERE id = {notif_id}"))
        session.commit()
...
async def check_tournaments():
    try:
        script_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../daily_check.py'))
...
async def sync_omnipong():
    try:
        ...
        script_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../autoscrape.py'))
        cwd = os.path.dirname(script_path)
...
async def sync_stadium():
    try:
        ...
        script_path = os.path.abspath(os.path.join(os.path.dirname(__file__), 'stadium_scraper.py'))
...
def get_agent_context():
    ...
    try:
        # 1. User Profile
        user_res = session.execute(text("SELECT name as full_name, current_rating as rating, usatt_id as usatt_number FROM users LIMIT 1")).fetchone()
        user_data = dict(user_res._mapping) if user_res else {}

        # 2. Recent Matches (Last 5)
        matches_res = session.execute(text("SELECT date, opponent_name, opponent_rating, result, score_summary, set_scores FROM matches ORDER BY date DESC LIMIT 5"))
        recent_matches = [dict(row._mapping) for row in matches_res]

        # 3. Upcoming Tournaments (Next 3)
        tourney_res = session.execute(text("SELECT title, location, date_range, status FROM activities WHERE activity_type='tournament' ORDER BY id DESC LIMIT 3"))
...
def get_stats(source: str = "omnipong"):
    ...
    try:
        ...
        tourney_query = text(f"SELECT COUNT(DISTINCT date) FROM matches WHERE {source_filter}")
        ...
        return {
            ...
            "trend": trend,
            "rating_context": "USATT Official" if source == "usatt" else ("Arcade Mode" if source == "arcade" else "Club League"),
            "patterns": {
                "comebacks": comebacks,
                "chokes": chokes,
                "close_game_win_rate": round(close_sets_won / close_sets_swung * 100) if close_sets_swung > 0 else 0
            }
        }
    except Exception as e:
...
async def tool_sync_league():
    ...
    try:
        script_path = os.path.abspath(os.path.join(os.path.dirname(__file__), 'stadium_league_scraper.py'))
        ...
        if process.returncode == 0:
            return {"status": "success", "message": "League match history synced successfully", "output": stdout.decode()}
        else:
...
async def tool_sync_league_players():
    ...
    try:
        ...
        script_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../sync_omnipong_players.py"))
        ...
        if process.returncode == 0:
            return {"status": "success", "message": "League players synced successfully", "output": stdout.decode()}
        else:
            return {"status": "error", "message": f"Sync failed: {stderr.decode()}"}
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
async def tool_sync_tournaments(scope: str = "regional"):
    ...
    """
    try:
        # We need a new script or just reuse autoscrape structure?
        # Let's verify if we need to run this in-process or subprocess.
        # Subprocess is safer for long running tasks to avoid blocking main thread event loop?
        # Actually, since main loop is async, we can run it here if we use asyncio properly.
        # But for stability, let's create a small wrapper script 'sync_tournaments.py'
        
        script_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../sync_tournaments.py"))
        
        process = await asyncio.create_subprocess_exec(
            "python3.10", script_path, "--scope", scope,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            cwd=os.path.dirname(script_path)
        )
        ...
        if process.returncode == 0:
            return {"status": "success", "message": f"Tournament sync ({scope}) complete", "output": stdout.decode()}
        else:
            return {"status": "error", "message": f"Sync failed: {stderr.decode()}"}
    except Exception as e:
        return {"status": "error", "message": str(e)}
@app.post("/tools/query/matches")
def tool_query_matches(opponent_name: str = None, date_from: str = None, result: str = None):
    session = SessionLocal()
    try:
...
def tool_search_players(req: PlayerSearch):
    ...
    try:
        ...
        if not players and name:
            match_res = session.execute(text("SELECT DISTINCT opponent_name as name, opponent_usatt_id as usatt_id, opponent_rating as rating FROM matches WHERE opponent_name LIKE :name AND opponent_rating IS NOT NULL"), {"name": f"%{name}%"})
            players = [dict(row._mapping) for row in match_res]
            
        if not players:
            return {"status": "not_found", "message": f"No player found locally matching '{name}'"}
...
def save_arcade_match(session, opponent_name, result, score_summary, set_scores, date_obj=None):
    from models import Match, Player
    from datetime import datetime
    
    # 1. Find/Create Opponent
    opp_rating = 1200
    existing_opp = session.execute(text("SELECT rating FROM players WHERE name = :n"), {"n": opponent_name}).fetchone()
...
def arcade_lookup_player(req: PlayerSearch):
    ...
    try:
        ...
        player = session.execute(text("SELECT * FROM players WHERE name LIKE :name"), {"name": name}).fetchone()
        
        if player:
            return {"status": "found", "player": dict(player._mapping)}
        else:
            # Check matches for history
            history = session.execute(text("""
                SELECT AVG(opponent_rating) as est_rating, COUNT(*) as match_count 
                FROM matches 
                WHERE opponent_name LIKE :name AND opponent_rating IS NOT NULL
            """), {"name": name}).fetchone()
            
            est_rating = 1200
            if history and history.est_rating:
                est_rating = int(history.est_rating)
            ...
            return {
                ...
                "message": "Player not in database, estimated rating provided."
            }
    except Exception as e:
        return {"error": str(e)}
    finally:
...
def get_practice_partners(limit: int = 5):
    ...
    try:
        ...
        for opp_name, stats in opponent_stats.items():
            ...
            if rating_diff <= 50: skill_score = 1.0
            
            # 2. Competitiveness Score (Percent of close matches)
            comp_score = min(1.0, close_match_rate * 1.5) # Boost it a bit so 33% close is 0.5 score
            ...
            total_weighted_score = (skill_score * 0.3) + (comp_score * 0.3) + (balance_score * 0.3) + (recent_score * 0.1)
...
def get_tool_analysis():
    ...
    try:
        ...
        result = session.execute(text("SELECT date, opponent_name, result, set_scores FROM matches WHERE set_scores LIKE '%11-9%' OR set_scores LIKE '%9-11%' LIMIT 3"))
...
CLAUDE_TOOLS = [
    ...
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "source": {"type": "string", "enum": ["usatt", "club"], "description": "The data source."}
            }
        }
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "opponent_name": {"type": "string", "description": "Search for a specific opponent."},
    ...
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "name": {"type": "string", "description": "The player's name or USATT ID."}
            }
        }
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "player_name": {"type": "string", "description": "The name of the player to search for (e.g., 'Dawson, Jerry')."}
            },
